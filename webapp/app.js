(function () {
  ...
  document.querySelector('#tabbar [data-tab="stats"] span').textContent = EN?"Stats":"Статистика";
  ...
  function syncDraft(patch, immediate) {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (seq !== syncSeq || !res || !res.ok) return;
        state = res;
        if (!$("v-result").classList.contains("hidden")) renderCard();
      } catch(e) {}
    };
