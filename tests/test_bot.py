...
class MenuLifecycleTests(unittest.IsolatedAsyncioTestCase):
    async def test_first_visit_brand_art_is_sent_once_and_never_replaces_menu(
        self,
    ) -> None:
        class AnimatedPrivateMessage(PrivateMessageStub):
            ...
            async def reply_photo(self, *, photo, caption: str) -> None:
                self.animations.append((photo.read(), caption))
        ...
        self.assertTrue(message.animations[0][0].startswith(b"\x89PNG"))
        self.assertIn("Твоя музыка", message.animations[0][1])
        ...
        self.assertEqual(
            next(
                ...
                for button in row
                if button.text == "Попробовать на примере"
    ...
    async def test_failed_first_visit_animation_can_retry_later(self) -> None:
        class FailingAnimatedMessage(PrivateMessageStub):
            async def reply_photo(self, **_kwargs) -> None:
    ...
    async def test_repeated_start_is_debounced(self) -> None:
        ...
        self.assertIn("Музыка, которой хочется делиться", message.replies[-1])
    ...
    async def test_help_sends_visible_reply_and_retires_home(self) -> None:
        ...
        self.assertIn("Как пользоваться", message.replies[-1])
    ...
    async def test_start_forgets_an_expired_active_card(self) -> None:
        ...
        self.assertNotIn("Продолжить карточку", labels)
...
class BotKeyboardTests(unittest.TestCase):
    ...
    def test_profile_descriptions_are_current_and_fit_telegram_limits(self) -> None:
        ...
        self.assertIn("подборка", BOT_SHORT_DESCRIPTIONS[""])
    ...
    def test_release_keyboard_can_hide_channel_button(self) -> None:
        ...
        self.assertEqual(button_texts, ["🎧 Слушать трек", "🟢 Spotify"])
    ...
    def test_release_keyboard_uses_two_columns(self) -> None:
        ...
        rows = keyboard.inline_keyboard
        self.assertEqual(rows[0][0].text, "🎧 Слушать трек")
    ...
    def test_editorial_ui_mode_uses_livelier_hub_copy(self) -> None:
        ...
        self.assertEqual(keyboard.inline_keyboard[0][0].text, "💿 Слушать альбом")
    ...
    def test_podcast_release_keyboard_uses_podcast_hub_label(self) -> None:
        ...
        self.assertEqual(keyboard.inline_keyboard[0][0].text, "🎙️ Слушать подкаст")
    ...
    def test_section_keyboard_offers_related_pages_and_back_last(self) -> None:
        ...
        self.assertEqual(
            ...
            ["v2|menu|guide", "v2|menu|platforms"],
        )
        self.assertEqual(
            [button.callback_data for button in rows[1]],
            ["v2|menu|demo", "v2|menu|onboard1"],
        )
        self.assertEqual(rows[-1][0].callback_data, "v2|menu|start")
    ...
    def test_home_text_is_personal_and_escapes_telegram_html(self) -> None:
        ...
        self.assertIn("StonerHand, &lt;Артём&gt;", text)
        self.assertNotIn("Подборка · 4/10", text)
        self.assertIn("ссылку", text)
        self.assertIn("<code>артист — название</code>", text)
        self.assertIn("подводка", text)
    ...
    def test_first_visit_home_explains_every_supported_input_shape(self) -> None:
        ...
        self.assertIn("Музыка, которой хочется делиться", text)
        self.assertIn("ссылку", text)
        self.assertIn("<code>артист — название</code>", text)
...
class InlineModeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_classic_inline_collection_uses_collage_preview(self) -> None:
        ...
        self.assertTrue(
            result.input_message_content.link_preview_options.url.startswith(
                "https://i.scdn.co/"
            )
        )
        self.assertTrue(result.input_message_content.message_text)
    ...
    async def test_inline_collection_recovery_survives_a_later_user_action(
        self,
    ) -> None:
        from music_links_bot.bot_inline import inline_query_handler
        from music_links_bot.bot_runtime import BotRuntime
        ...
        urls = [
            "https://open.spotify.com/track/4x53Xb3mAbSOzAq1h9YbuU?si=first",
            "https://open.spotify.com/track/0WC0jiILJRkb6P041h4jiU?si=second",
            "https://open.spotify.com/track/3pCoUI2da3hhMQUqZLYlir?si=third",
            "https://open.spotify.com/track/5Cj1UNdBWSuDyFjiF19mLa4?si=fourth",
            "https://open.spotify.com/track/1yLW4FiuCJG69WPNpz4bl9?si=fifth",
            "https://open.spotify.com/track/7eiC7fCOH73X1lpeFuulef?si=sixth",
        ]
        expected = [url.split("?", 1)[0] for url in urls]
        runtime = BotRuntime()
        await runtime.remember_collection(77, urls=urls, lang="ru")
        await runtime.remember_action(
            77,
            kind="search",
            value="unrelated later action",
            lang="ru",
        )
        context = ContextStub()
        context.application.bot_data["runtime"] = runtime
        ...
        self.assertEqual(build_result.await_args.args[0], expected)
        self.assertEqual(inline_query.answers[0][0], [complete])

    async def test_inline_collection_recovery_never_uses_another_users_state(
        self,
    ) -> None:
        from music_links_bot.bot_inline import inline_query_handler
        from music_links_bot.bot_runtime import BotRuntime
...
class BotLookupTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_private_messages_without_supported_urls_get_hint(self) -> None:
        ...
        self.assertIn("<blockquote>привет</blockquote>", message.replies[0])
        ...
        self.assertEqual(keyboard[1][0].text, "Отправить прямую ссылку")
