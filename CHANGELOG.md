...
- Восстановлена независимая от сервера коллажей inline-отправка: по умолчанию текстовая карточка с исходной обложкой. Фото с подписью доступны только через экспериментальный флаг; принудительный классический режим всегда возвращает текстовую карточку.

- Исправлен пустой блок вместо обложки: короткие публикации и inline-подборки отправляются фотографией. Для обычных публикаций изображение загружается прямо в Telegram; длинные тексты и отклонённые фото используют исходное превью площадки. Компактный режим отключён для сгенерированных изображений.

- Подпись канала доступна в inline и обычных ссылочных превью; подборки с общей обложкой и более чем шестью релизами тоже используют брендированное изображение. Обновлена версия URL для обхода старого кеша Telegram.

## 1.21.0 — 2026-09-23

- Кеш готовых брендированных обложек и Telegram file_id, объединение одинаковых
  запросов и переиспользование HTTP-соединений с корректным закрытием.
- Общий план подборки для чата, inline и предпросмотра; проверка неполных данных,
  отсутствующих обложек и повторов внутри подборки, подсказки перемещения/удаления.
- Причины отказа провайдеров и очередь с задержкой, числом попыток и причиной сбоя.
- Измерения p50/p95 поиска и очереди, ограниченные последними 200 значениями.
- Сброс оформления перенесён в «Ещё». Сценарии поиска/ошибок выделены из bot.py,
  типизация распространена на 11 модулей, обновлены README и руководство.
- Tidal принимает объект или список исполнителей; поддерживаются совместные релизы.


- Обложки обычных постов и коллажи подборок используют единую подпись канала: шрифт, отступы и нижний градиент. Настройки BRAND_LABEL и BRAND_PHOTO_FRAME общие; новые адреса превью обходят кеш старых коллажей.

- Webhook учитывает ошибки реального диспетчера PTB, ограничивает параллельную
  обработку восемью updates и разделяет выполнение, завершение и неоднозначную
  доставку. Перед отправкой сохраняется маркер Redis; повтор после потери ответа
  не создаёт новую публикацию. При недоступной дедупликации запрос можно повторить.
- «Трек + видео» сохраняет авторскую подводку и настройки тегов в Rich-формате;
  сетевые ошибки Rich, альбома и ephemeral-ответов не запускают слепой fallback.
- Privacy удаляет временные записи и старые черновики других экземпляров,
  использует индекс владельца и запрещает записи от запросов до удаления.
  Selection и retry соблюдают TTL также в локальной памяти.
- Ошибки хранилища очереди возвращают HTTP 503. Время успешного tick хранится
  отдельно от попытки. Standalone polling запускает и останавливает свой worker.
- Статистика считает уникальные события атомарно в Redis вместо объединения
  локальных снимков через max. Runtime-метрики явно обозначены как данные экземпляра.
- Файловая статистика и композиция обложки вынесены из event loop; одновременно
  компонуются не более двух обложек на цикл. Добавлены интеграционные проверки
  PTB и Redis Lua, включая неоднозначную доставку и удаление между запросами.

## 1.20.2 — 2026-09-14

- Исправлена регрессия карточек 1.20.1: ссылки на альбом и артиста больше
  не принимаются за случайно оставленные исходные URL. Исправление действует
  в редакторе, публикации, inline по ссылке и поиске по названию.
- Deezer получает данные трека или альбома через собственный публичный API;
  короткие ссылки раскрываются с проверкой каждого перехода.
- Tidal и Bandcamp получают точные сведения с публичной страницы релиза.
  Яндекс Музыка поддерживается при доступности метаданных; региональная
  заглушка не превращается в выдуманный релиз.
- YouTube Music использует официальный YouTube oEmbed, сохраняя исходную ссылку.
- Добавлена проверка полного пути «ссылка → карточка → inline» для всех
  заявленных музыкальных сервисов, трёх Apple-ссылок из отчёта и запроса
  «кино — пачка сигарет». Региональные отказы явно показываются отдельно.

## 1.20.1 — 2026-09-14

- Чистая обложка стала исходным выбором для новых музыкальных карточек:
  публикации из редактора отправляются с фото, inline и ответы в чатах используют
  изображение обложки в превью. Сохранённый выбор площадочного превью, старые
  черновики, пользовательские обложки и видеопревью не переписываются.
- Название альбома и имя исполнителя становятся ссылками при наличии
  подтверждённых адресов Spotify или Apple Music. Ссылка на альбом Apple Music
  открывает альбом целиком, без выделения исходного трека.
- Рядом с типом, годом и количеством треков отображается длительность.
  Для альбомов сумма считается только по полному составу с известной длиной
  каждой позиции. Дополнительные запросы ограничены по времени; отсутствие
  сведений не мешает получить карточку.
- У сборников Various Artists убрана повторяющаяся строка исполнителя.
  Видео, радио, плейлисты и профили артистов используют единые заголовки,
  отступы и подписи источника на русском и английском.
- При явном отказе Telegram загрузить обложку площадки остаётся обычная
  карточка. Неопределённая сетевая ошибка не запускает повторную отправку.

## 1.20.0 — 2026-09-13

- Гостевой вызов по упоминанию бота: ссылка в сообщении или ответе превращается
  в карточку без открытия редактора. Неоднозначный поиск предлагает выбрать
  релиз. Включается через Guest Mode в BotFather.
- «О релизе» открывает площадки и копирование названия или ссылки. В группах
  панель видна только нажавшему; у inline-карточек открывается личный чат бота.
  Пользовательские черновики и подводки не входят в общую панель.
- «Стоп» действительно отменяет запрос к провайдерам, если его больше никто
  не ожидает. Уже найденные позиции остаются доступны в исходном порядке.
  Отмена одного пользователя не прерывает такой же поиск другого.
- `/stats` показывает самые часто отправляемые inline-карточки за неделю.
  Учитываются события отправки после включения `/setinlinefeedback`, без
  поисковых запросов и получателей; повторные updates не удваивают счётчик.
- Радиошоу и станции Apple Music обрабатываются отдельно от треков и альбомов.
  Исправлена ссылка The Alligator Hour: название, тег `#radio` и кнопка Apple Music.
  Страница обычного куратора не выдаётся за подтверждённое радиошоу.
- Обновлены список событий webhook и polling, справка по настройке и сведения
  о хранении данных. Оформление карточек с разделителем «·» сохранено.

## 1.19.2 — 2026-09-13

- Вернули разделитель между значком и названием релиза: «🎧 · Dig».
  Остальное оформление карточки из 1.19.1 сохранено.

## 1.19.1 — 2026-09-13

- Заголовок карточки начинается с небольшого значка и жирного названия релиза;
  исполнитель находится на следующей строке. Тип и подтверждённый год отделены
  пустой строкой и больше не выделяются курсивом.
- У трека показывается «💿 Из альбома „…“», когда название альбома есть в данных
  релиза. Неизвестный альбом пропускается; тег `#album` к треку не добавляется.
- Обычные карточки, включая inline, используют стандартные сообщения Telegram.
  Rich доступен для отдельно выбранного лонгрида. Сохраняются выбранные обложки,
  видеопревью, кнопки, ручные теги и полный список подборок.
- Добавлены 12 проверок нового оформления, экранирования метаданных, длинных
  названий и подводок, отправки фото и работы без обложки. Всего: 838 тестов
  и 104 подтеста. Ранее отправленные сообщения автоматически не изменяются.

## 1.19.0 — 2026-09-12

- Карточка получила компактный рабочий экран: изменение, добавление в подборку,
  превью, сохранение и отправка. Текст, теги, обложка и кнопки доступны через
  «Изменить»; редкие настройки и шаблоны остаются в «Ещё».
- «Мои посты»: фильтры, поиск, закреплённые сохранения, срок хранения каждой
  карточки. Обычные черновики — 7 дней, сохранённые — 90 дней, до 30 сохранений.
  История поиска доступна отдельной кнопкой.
- Подборка доступна с главного экрана; управление позицией раскрывается после
  выбора. Добавлены явное добавление ссылок и замена выбранного релиза.
  Нумерация, полный список и лимит десяти позиций сохранены.
- Черновики, сессии и состав подборки обновляются сравнением версий в Redis.
  Независимые правки настроек объединяются; конфликты не затирают данные.
  Кнопки редактора и подборки проверяют актуальность состояния.
- Новые ссылки во время правки текста получают выбор назначения. Повторное
  нажатие выбора не запускает повторный поиск. Новая подборка не изменяет старую.
- Повтор частичного поиска использует сохранённые успешные результаты и запрашивает
  только остальные источники, сохраняя порядок.
- Ручная отправка записывает намерение и подтверждение доставки. Неопределённый
  результат не повторяется автоматически; повтор требует проверки чата.
  Замена удаляет старую публикацию только после подтверждения новой.
- «Исходный вид», отмена правок до пяти минут и объяснение выбранных тегов.
  Произвольные пользовательские теги не выдаются за общий жанр подборки.
- Обновлены RU/EN-тексты, документация, локальный макет и регрессионные проверки.
  Подробнее: [отчёт 1.19](docs/reports/release-1.19.0.ru.md).

## 1.18.2 — 2026-09-11

- Черновики, настройки, подборки и шаблоны читаются из актуального хранилища.
  Тёплый сервер больше не подменяет их старой копией из памяти. Сессия
  повторно используется только внутри одного запроса, включая inline-режим.
- Сохранение черновиков, настроек и подборок требует подтверждения Redis.
  При сбое бот сообщает о проблеме; чтение не выдаёт ошибку соединения за
  отсутствие данных. Локальная работа без Redis остаётся доступна.
- Блокировка отправки не переходит в локальный режим при отказе Redis;
  проверка ранее опубликованного поста также требует ответа хранилища.
- Заполненная подборка честно сообщает о лимите 10 позиций и не помечает
  одиннадцатый релиз как добавленный.
- Загрузка обложки для необязательной брендированной рамки ограничена
  проверенными CDN. Неизвестный адрес сохраняет обычную карточку без рамки.
- Исправлена минимальная версия сборщика для SPDX-лицензии. Добавлена
  проверка готового wheel: импорты, переводы, оригинальная аватарка и лицензия.
- Подробности ревью и границы проверки: [отчёт 1.18.2](docs/reports/release-1.18.2.ru.md).

## 1.18.1 — 2026-09-11

- Главная кнопка называет материал: «🎧 Слушать трек», «💿 Слушать альбом»,
  EP, сингл, сборник, саундтрек или подкаст. Она открывает страницу релиза
  с доступными сервисами прослушивания.
- Возвращены небольшие значки площадок, видео, радио и действия «Поделиться».
  Главное действие остаётся единственным цветовым акцентом; режим Minimal
  сохраняет текстовые подписи музыкальных ссылок без значков.
- Названия и номера позиций в подборках сохранены. Обновление применяется
  к новым карточкам; ранее отправленные сообщения автоматически не меняются.

## 1.18.0 — 2026-09-09

- Текст, теги, обложка и кнопки доступны сразу под постом. Превью открывается
  отдельно; редкие действия, шаблоны и форматы перенесены в «Ещё».
- Главный экран содержит создание поста, продолжение черновика, «Мои материалы»
  и настройки. История поиска, черновики и подборка собраны в одном разделе.
- Публикация сейчас, планирование и отправка себе находятся на одном экране
  с указанием канала. Старые кнопки продолжают работать.
- Настройки разделены на язык, новые посты и подборки. Изменения текущего
  черновика больше не перезаписывают шаблон для следующих постов.
- Выбор обложки релиза или превью убирает ранее загруженное изображение;
  отмена восстанавливает его. Из настроек подборки можно вернуться к ней.
- Сокращены и согласованы тексты RU / EN; исправлены буквальные \n вместо
  переносов строк в части подсказок. Дополнительные действия с тегами
  показываются отдельно и учитывают сохранённые исправления релиза.

## 1.17.0 — 2026-09-09

- Автотеги различают трек, альбом, EP, сингл, официальный сборник, саундтрек,
  авторскую подборку и видео. Тип публикации имеет приоритет; до пяти тегов
  нормализуются и очищаются от дублей. Жанр подборки используется только
  при согласованных данных всех музыкальных позиций.
- В редакторе появились отдельные переключатели тегов, добавление своих,
  сохранение исправлений для конкретного релиза и возврат к автоматическому
  подбору. Правки не переносятся на другие релизы или других пользователей.
- Удаляются служебные подписи Spotify. Сохраняются названия саундтреков,
  концертов и ремастеров. Подтверждённые формат, год и количество треков
  показаны отдельной строкой; у Various Artists первым идёт название сборника.
- Кнопки используют ясные названия без цветных кружков. Универсальная ссылка
  подписана «Выбрать площадку»; одинаковые адреса объединяются. У подборки
  сохраняются номера, полные подписи кнопок и порядок позиций.
- `/settings` сохраняет режим «Одна колонка / Авто / Компактно», разделы по
  артистам или альбомам и выбор чистой обложки либо превью площадки.
  YouTube сохраняет воспроизводимое превью. Исходная аватарка не изменена.
- Подборки содержат полный нумерованный список и точный состав. Можно добавить
  заметку или раздел к позиции; они следуют за релизом при перестановке.
  Десять позиций остаются одним постом. При превышении лимита сначала
  сокращаются заметки; последние позиции не удаляются.
- Inline-результаты персональные и обновляются сразу после изменения настроек.
  Собственное название подборки сохраняется при отправке через inline.
- Расширены проверки перезапуска, конкурентных запросов, чужих кнопок,
  противоречивых жанров, HTML, длинных списков и опубликованной версии бота.

## 1.16.0 — 2026-09-08

- Упрощены первый запуск и редактор: оформление, текст и кнопки площадок
  открываются отдельно, а меню отправки содержит только варианты доставки.
- Сохранена исходная аватарка StonerHand с когтистой рукой; она используется
  в приветствии. В карточке артист выделен жирным, название стоит отдельной
  строкой, исходные обложки релизов и Classic-подборки сохранены.
- `/settings` сохраняет язык интерфейса, оформление новых карточек и хэштеги.
  Настройки переживают перезапуск и изолированы между пользователями.
- Черновики и недавние релизы листаются по пять. В библиотеке доступно до
  30 черновиков с названиями и статусами очереди или публикации.
- Планирование доступно прямо на карточке администратора. Кнопки содержат
  точные дату и время с указанным часовым поясом; устаревшее время отклоняется,
  а повторное нажатие не добавляет второе задание на тот же момент.
- После публикации показаны канал, дата и кнопка «Открыть пост».
- Добавлены регрессии настроек, параллельной обработки языков, библиотеки
  и планирования. Локальный интерактивный просмотр собирается командой
  `python scripts/render_ui_preview.py`.


## 1.15.3 — 2026-09-08

- Усилена проверка секретов webhook и подписей коллажей: некорректные строки
  отклоняются без ошибки сервера. Загрузка брендированных обложек ограничена
  по времени, объёму и числу пикселей.
- Публикация, повтор и замена одного черновика используют общую блокировку;
  отмена задачи корректно её освобождает. Старые кнопки и запросы ввода больше
  не изменяют удалённые черновики.
- Временные ошибки проверки прав канала не кэшируются, а отмена одного
  запроса MusicBrainz не прерывает поиск для других пользователей.
- Превью доступно прямо на карточке. Очередь показывает активную отправку,
  подтверждает отмену и неоднозначный повтор, сохраняет страницу при возврате
  и спокойно обрабатывает повторное обновление неизменившегося списка.
- Добавлены регрессионные проверки безопасности, конкурентных действий
  и восстановления очереди; подробности — в [локальном аудите](docs/reports/quality-1.15.3.ru.md).

## 1.15.2 — 2026-09-05

- Полным Classic-подборкам из 2–6 релизов возвращён единый квадратный коллаж:
  число обложек теперь соответствует числу найденных релизов, если у каждого
  есть отдельное безопасное изображение.
- Текст, список, кнопки и сохранённая подводка остаются в Classic-формате;
  Rich-рендер для подборок не возвращался. Partial-наборы и подборки с одной
  доступной обложкой безопасно используют preview первого релиза.
- Inline-cache и production smoke-контракт обновлены, а live canary теперь
  загружает collage URL и проверяет настоящий JPEG до отправки карточки.

## 1.15.1 — 2026-09-05

- Авторская подводка больше не теряется у сообщений с несколькими музыкальными
  ссылками: поддерживаемые URL удаляются из текста, а остальное форматирование
  сохраняется перед списком релизов.
- Подводка полной приватной подборки хранится вместе с точным упорядоченным
  набором источников и восстанавливается после serverless-перезапуска при
  публикации через «Поделиться». К другой подборке этот текст не подмешивается.
- Подборкам возвращён прежний Classic-вид с крупным preview первого релиза.
  Автоматический Rich-рендер и коллаж больше не меняют вид готового поста;
  ранее опубликованные collage-URL продолжают обслуживаться.
- Общий UTF-16 budget теперь применяется к подводкам всех типов подборок:
  слишком длинный лид аккуратно сокращается, но заголовок, полный список и
  хэштеги всегда остаются в итоговой публикации.
- Inline-result version повышена, а регрессии покрывают прямой пост, кнопку
  «Поделиться», точное сопоставление подборки, новую serverless-инстанцию и
  старое preview первого релиза.

## 1.15.0 — 2026-09-05

- Исключена слепая повторная отправка после потерянного ответа Telegram: очередь
  помечает начатую доставку до сетевого вызова, автоматически повторяет только
  подтверждённые `400/403/429` и выносит неоднозначный результат на ручную
  проверку.
- `/api/health` стал строго read-only. Отправка очереди вынесена в отдельный
  защищённый пятиминутный workflow; последний tick и сомнительные доставки
  видны в health и делают production canary красным.
- Каждая кнопка площадки теперь обязана вести на HTTPS-домен именно этой
  площадки. Закрыты подмена хоста, userinfo, нестандартные порты, кодированная
  страница поиска, битые URL и старый регистр ключей сохранённых карточек.
- Коллажи ограничены по размеру каждого файла, общему объёму и числу пикселей;
  исходники уменьшаются сразу после декодирования, а одновременно загружаются
  не более двух обложек. Полные названия релизов в кнопках больше не обрезаются.
- Усилен ключ защиты от повторов: учитываются канал, тип релиза и точный источник,
  а старые записи читаются во время миграции. Добавлен opt-in Telegram canary,
  отправляющий и удаляющий Classic, Rich, collection и SoundCloud-only карточки.
- Релизный gate расширен branch coverage с порогом 72%, постепенной mypy-проверкой
  и регрессиями ошибок провайдеров, очереди, ссылок и памяти коллажей. Живой
  provider canary теперь проходит пользовательские сценарии `Deftones — Rickets`
  и SoundCloud-only без выдуманной кнопки Spotify.
- Единый строгий нормализатор теперь применяется и при объединении одинаковых
  релизов: старые ключи площадок мигрируют, а ссылка чужого домена не может
  попасть в селектор, share-запрос или финальную кнопку. Автооткат отличает
  дефект нового build от уже существующего ручного состояния очереди.
- Прямой SoundCloud-источник больше не проходит через угадывание Songlink:
  метаданные и кнопка берутся только у SoundCloud, а версия общего lookup-кеша
  повышена, чтобы старый ложный Spotify-match не пережил развёртывание.

## 1.14.1 — 2026-09-04

- Текстовый и Apple Music fallback теперь восстанавливает прямую Spotify-ссылку
  через точный Deezer ISRC и адресную MusicBrainz-связь; строгий поиск по артисту
  и названию остаётся резервом. Поиск
  `Deftones — Rickets` снова получает Spotify, Apple Music и универсальный
  `song.link`, построенный от подтверждённого Spotify ID.
- Apple-only результат больше не выдаётся за полный список площадок: если
  Spotify или готовая универсальная страница не подтверждены, ложная кнопка
  **Все платформы** скрывается, а доступная Apple-ссылка остаётся основной.
- При отсутствии legacy API key бот больше не тратит время на гарантированные
  анонимные 401 Songlink API. Резервные обработчики подборок запускаются
  параллельно и сохраняют полный порядок результатов.
- Независимый provider canary теперь проверяет живую связь
  Deezer ISRC → MusicBrainz → Spotify; добавлены регрессии точного сопоставления, типов URL,
  кэширования, честной CTA-иерархии и полного lookup-пути.

## 1.14.0 — 2026-09-04

- Главный экран получил явную навигацию: редактируемые **Черновики** отделены
  от истории **Недавних**, а редко используемые параметры собраны в
  **Настройках**. Активная карточка и подборка остаются доступны в один шаг.
- Универсальная ссылка Songlink/Odesli теперь всегда идёт первой полноширинной
  primary-кнопкой; прямые сервисы остаются вторичными быстрыми ссылками. Та же
  иерархия сохранена в карточках, предпросмотре и редакторе.
- Кнопки всех типов подборок стали адаптивными: короткие названия занимают по
  две колонки, длинные автоматически переходят на отдельную строку без грубого
  обрезания.
- Ошибка поиска показывает исходный запрос и два понятных пути восстановления:
  изменить формулировку или отправить прямую ссылку.
- Текст кнопки публикации очищен от дублирующей стрелки, которую Telegram уже
  показывает нативно. Добавлены регрессии разделения истории и черновиков;
  production UI/UX smoke-контракт обновлён до версии 4.

## 1.13.7 — 2026-09-01

- Последняя полностью обработанная подборка теперь хранится отдельным durable-снимком,
  поэтому следующий поиск, действие редактора, перезапуск или устаревшая serverless-
  инстанция не могут снова сократить список при inline-публикации.
- Все inline-запросы и callback-данные проверяются на общем уровне создания кнопок и
  финальным контрактом публикации. Сырые подборки и значения сверх лимитов Telegram
  блокируются, а Rich-режим больше не маскирует ошибку молчаливым обрезанием.
- Публичная замена стала атомарной: при сбое хотя бы одной ссылки бот не отправляет
  укороченную подборку и не удаляет исходное сообщение.
- `/privacy` удаляет новый durable-снимок вместе с сессией. Добавлены регрессии для
  шести ссылок, изоляции пользователей, перезапуска, гонки инстанций, частичной
  публичной публикации и лимитов кнопок.

## 1.13.6 — 2026-09-01

- Длинный старый inline-запрос, который Telegram Bot API обрезает на границе
  256 символов, теперь восстанавливается из последней приватной операции того же
  пользователя. Полный набор из шести–десяти ссылок больше не превращается в
  тупик «не все ссылки обработаны».
- Восстановление требует точного совпадения ссылок и их порядка; данные другого
  запроса или пользователя не подмешиваются.
- Компактные share-запросы остаются основным путём, а понятная подсказка показывается
  только когда безопасно восстановить обрезанный список действительно невозможно.

## 1.13.5 — 2026-09-01

- Inline-публикация восстанавливает трек, временно потерянный внешним резолвером, из
  сохранённой подборки её владельца. Уже полностью собранные 4–10 релизов больше
  не деградируют в `3 из 4` из-за повторного provider lookup.
- Восстановленный inline-ответ помечается персональным и не попадает в общий Telegram-кэш.
- Строгий барьер 1.13.4 сохранён: если нет ни provider-ответа, ни сохранённых метаданных,
  неполную карточку всё равно нельзя отправить.

## 1.13.4 — 2026-09-01

- Неполная inline-подборка больше не является выбираемым постом. Если хотя бы одна из
  переданных ссылок не обработана, Telegram получает только действие возврата к
  подборке, а карточку `3 из 4` больше нельзя отправить.
- Добавлена регрессия строгого inline-контракта: неполный результат не кэшируется и не
  публикуется.

## 1.13.3 — 2026-09-01

- Подборка теперь получает штатную кнопку «Поделиться» и передаёт все
  релизы одним компактным inline-запросом вместо набора длинных URL.
- Добавлена защита от 256-символьного лимита Telegram: обрезанный список
  ссылок больше не публикуется как ложно полная подборка.
- Classic preview собирает коллажи для 2–6 релизов; для шести обложек используется
  сбалансированная сетка 3×2.
- Добавлены регрессии для полного share-query, отсечения обрезанного inline-ввода,
  коллажа из шести обложек и его production health-контракта.
