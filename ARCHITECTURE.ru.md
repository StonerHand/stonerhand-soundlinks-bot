...
3. **Тёплый reuse**: приложение PTB, HTTP-пулы и кеши создаются один раз на инстанс (`_ensure_application`). Обработка каждого update ограничена по времени (`asyncio.wait_for`), так что зависший upstream не держит общий лок и не вешает инстанс. Приложение утилизируется только после нескольких сбоев подряд — один кривой update не холодит следующего пользователя.
...
Визуальный редактор без сборки: семантический `index.html`, отдельные `styles.css` и `app.js`. Статика — `/app`, API — `POST /api/webapp` с `{init_data, action, payload}`. Подпись `initData` проверяется официальным HMAC-алгоритмом (`webapp_auth.py`), свежесть 24 ч.
...
Дизайн: шрифты Unbounded (заголовки) + Golos Text (текст) + JetBrains Mono (лейблы), тёплая янтарная палитра, плёночное зерно. Светлая и тёмная темы через CSS-переменные — тема берётся из `tg.colorScheme`, ручной переключатель хранится в CloudStorage. Навигация — нижний таб-бар (Главная / Подборка / Очередь / Статистика; последние две только для админа). Компоновка фиксируется в `100dvh`, контраст светлой темы соответствует WCAG AA, цели касания не меньше 44 px, поддержаны keyboard focus, семантические dialog/navigation/status и `prefers-reduced-motion`.

UI: экран поста (обложка с градиентом, бейдж жанра, play-кнопка с кольцом прогресса и эквалайзером — аудио догружается лениво, цветные пилюли платформ), липкий док действий (Опубликовать + отложка/подборка/шаринг), полноэкранное «Оформление» с секциями-карточками, тумблерами и **пресетами** (сохранённые наборы в CloudStorage), **пакетная вставка** (2+ ссылки → авто-подборка), drag-and-drop в подборке (grip-ручка), перенос времени задания в очереди, share-picker (выбор конкретной площадки), плейлист-прослушка кандидатов (мини-плеер с ‹/›), иллюстрированные пустые состояния, моно-SVG иконки, переходы экранов, скелетоны, живой акцент из цвета обложки, undo-отсчёт после публикации, баннер вставки из буфера, typeahead из истории, хэптика, системная «Назад», онбординг, fullscreen (Bot API 8.0). **Устойчивость клиента:** каждый запрос ограничен таймаутом и отменяется при уходе с экрана (никаких зависаний), гонки отсекаются seq-гвардами, битая обложка падает в эмодзи-плитку, протухший `initData` показывает «сессия истекла». Все внешние URL проходят проверку схемы и экранирование (анти-XSS).

## Очередь публикаций (`publish_queue.py`)

Задания `{id, publish_at, attempts, draft}` в Redis (`queue:v1`; без Redis — память инстанса). Доставка **оппортунистическая**: тик после каждого telegram-update, на `GET /api/webapp` и на каждый пинг `/api/health` — монитор раз в 5 минут даёт точность до минут. Кросс-инстансовый лок `queue:lock` (SET NX, TTL 30 с) исключает двойную публикацию; мутации очереди (add/remove/reschedule) идут под тем же локом и при занятом локе возвращают явную ошибку вместо небезопасной записи. Лок освобождается атомарным compare-and-delete, поэтому просроченный владелец не удалит lease нового. **Ретраи:** созревшее задание сначала снимается с очереди, и если доставка упала — **возвращается с растущим backoff** и счётчиком попыток (до 3), и лишь потом дропается + алерт.
...
- **Троттлинг** `resolve`/`resolve_batch` пер-юзер (мягкий, пер-инстанс) защищает внешние API от спама.
...
└── app.js            состояние, API, навигация, редактор и аудио
...
tests/                270+ тестов: unittest, стабы клиентов, без сети
