...
src/music_links_bot/
  ...
  button_styles.py     семантика primary / success / danger и совместимость Bot API
...
Все inline-кнопки проходят через `button_styles.py`. На экране допускается одно главное действие `primary`; `success` используется для фактической отправки/публикации, `danger` — только для удаления. URL площадок, канал, шаринг, навигация и переключатели нейтральны. Новый Bot API-параметр передаётся через совместимый `api_kwargs`: актуальные клиенты показывают нативный цвет с учётом темы, старые безопасно используют стандартный фон.
