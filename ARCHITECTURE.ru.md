...
до шести кандидатов с исполнителем, названием, альбомом и годом. После выбора
...
Первый уровень — сетка «Студия / Найти / Подборка / Ещё»; счётчик подборки
живёт только на кнопке и не дублируется в тексте. Новому пользователю отдельно
показывается трёхшаговый тур. Справка и поддерживаемые сценарии открываются из
компактного подменю «Ещё». Очередь, публикация, аналитика и расширенное
...
ссылки в заголовке. Быстрая карточка ограничена сеткой 2×2: до двух
приоритетных площадок, универсальный hub и «Ещё». Нативная отправка с кнопками,
подборка и форматирование раскрываются только внутри «Ещё». Черновик из
...
подтверждается отдельно, а одиночное удаление можно отменить в течение пяти
минут. Crate и пользовательские сессии сохраняются в Redis с memory fallback.
...
- удаление карточки и очистка подборки требуют отдельного подтверждения;
