...
- **CI** (`.github/workflows/ci.yml`): pyflakes + 242 теста + `node --check` JS Студии на каждый пуш и PR.
...
Однофайловый визуальный редактор (vanilla JS, дизайн Vintage Amplifier). Статика — `/app`, API — `POST /api/webapp` с `{init_data, action, payload}`. Подпись `initData` проверяется официальным HMAC-алгоритмом (`webapp_auth.py`), свежесть 24 ч.
...
| `schedule` / `queue` / `unschedule` | админ | Отложенная публикация (очередь `queue:v1`) |
...
UI: липкий док действий (главная кнопка + отложка/подборка/шаринг), единый шит «Оформление» с амп-тумблерами, моно-SVG иконки, переходы экранов, скелетоны, живой акцент из цвета обложки, undo-отсчёт после публикации, баннер вставки из буфера, typeahead из истории, хэптика, системная «Назад», дефолты в CloudStorage, онбординг, fullscreen и ярлык на домашний экран (Bot API 8.0). Все внешние URL проходят проверку схемы и экранирование (анти-XSS).
...
webapp/
└── index.html        Mini App «Студия»: один файл, vanilla JS, Vintage Amplifier
...
tests/                242 теста: unittest, стабы клиентов, без сети
```
