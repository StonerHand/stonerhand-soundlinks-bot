...
- **CI** (`.github/workflows/ci.yml`): два джоба — `test` (pyflakes + 267 тестов + `node --check` JS Студии) и `e2e` (хедлесс-смоук Студии на Playwright: boot → поиск → карточка → подборка → ярлыки → пакетная вставка) на каждый пуш и PR.
...
| --- | --- | --- |
| `songlink.py` | Song.link / Odesli API | Кроссплатформенные ссылки, тип релиза, год, обложка. Резолвит Spotify, Apple Music, Deezer, Tidal, **Яндекс.Музыку**, **Bandcamp** и др. Параллельный опрос регионов, мерж ссылок. Кеш: локальный TTL + Redis (7 дней) |
