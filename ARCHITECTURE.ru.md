...
| --- | --- | --- | --- |
| Vercel webhook | `api/telegram.py` | Telegram отправляет POST updates на `/api/telegram` | Основной бесплатный serverless-деплой |
...
4. Убирает URL из пользовательского текста и превращает остаток в цитату
5. Делит ссылки через `_split_source_urls`
6. Отправляет ссылки в нужные клиенты
7. Форматирует ответ
8. В группах/каналах пробует удалить исходное сообщение и заменить его постом
9. Записывает счетчики в stats
...
| `prepend_user_text` | Делает цитату из текста над ссылкой |
...
| Кнопки платформ | По 2 в строку |
| Стили кнопок | `success` для Spotify, `danger` для YouTube, `primary` для хабов и остальных ссылок |
...
| Функция | Кнопки |
| --- | --- |
| `_build_link_keyboard` | Платформы для одного релиза + Song.link hub |
...
Порядок платформ зависит от `PRIMARY_PLATFORM`.
...
После деплоя нужно открыть:

```text
https://your-domain.vercel.app/api/set_webhook
```

Если есть `SET_WEBHOOK_SECRET`:
