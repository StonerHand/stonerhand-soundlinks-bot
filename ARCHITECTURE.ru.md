...
промах повторяется один раз. Неполный inline-результат имеет нулевой Telegram
cache time и предлагает повторно проверить исходные ссылки. Tracking-параметры
не создают отдельные записи aggregate-cache.
