...
src/music_links_bot/
  ...
  bot_recent.py        недавние карточки и повторное использование
  bot_editor_state.py  пресеты, статус и состояние быстрого редактора
  ...
  channel_templates.py последнее оформление каждого канала
...
- одинаковые одновременные Song.link-запросы, текстовые поиски и полные
  `LookupBundle` объединяются single-flight; завершившаяся задача удаляется
  даже после timeout/cancel вызывающего запроса;
...
В личке текстовый запрос сохраняется в пользовательской сессии. Бот показывает
до трёх кандидатов с исполнителем, названием, альбомом и годом. После выбора
или прямой ссылки одно сообщение обновляется по этапам, поэтому чат не
засоряется временными статусами. Если новый запрос отменяет предыдущий, старый
progress удаляется либо переводится в завершённое состояние. Любая неожиданная
ошибка также убирает зависший progress. Одинаковое пользовательское намерение
имеет короткий Redis/memory debounce: двойное сообщение или `/start` не создаёт
вторую карточку, но другой запрос сразу отменяет устаревший поиск.

`/start` рендерит короткий home-state из размера bot-crate и роли пользователя.
Первый уровень — «Создать / Подборка / Недавние / Студия»; счётчик подборки
живёт только на кнопке и не дублируется в тексте. Новому пользователю отдельно
показывается трёхшаговый тур. Справка остаётся в `/help`. Очередь, публикация,
аналитика и расширенное
оформление перенесены в Студию. Web App-кнопка
исключается в группах, где Telegram её не поддерживает.

Готовая карточка не содержит рекламных подписей или скрытой ссылки в заголовке.
Её автоматический footer состоит только из исполнителя, названия и хэштегов:
жанр, формат и год не дублируются в тексте, но жанр по-прежнему питает автохэштег.
Первая строка содержит основную площадку и универсальный hub, вторая —
«Изменить» и «В подборку». Быстрый редактор меняет то же сообщение и содержит
четыре группы настроек: стиль, текст, хэштеги и площадки. Отправка, публикация,
sharing и удаление раскрываются отдельно. Черновик из
текстового поиска хранит исходный запрос и позволяет показать другие варианты
либо изменить запрос без копирования текста. Цвета семантические: primary обозначает главное
действие, success — подтверждённый результат, danger — только разрушительное
действие. Для пересылки с кнопками Студия создаёт prepared message вместо
обычного Telegram forward.

Bot-crate рендерит список в тексте и до десяти коротких цифровых селекторов.
Стрелки и удаление показываются только для выбранного релиза. Полная очистка
подтверждается отдельно; удаление или очистку можно отменить в течение 15
секунд. Crate, активный draft и пять недавних draft ID сохраняются в Redis с
memory fallback.
...
- удаление карточки и очистка подборки требуют подтверждения и дают 15 секунд на отмену;
...
| `channel:template:v1:<hash>` | последнее оформление канала | 180 дней |
