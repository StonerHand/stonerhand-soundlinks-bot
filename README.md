...
finished before every source resolves. Public replacement is atomic: if one
source fails, the bot keeps the original message and never publishes a shortened
collection. Collection sharing uses one compact query generated by the bot. The
owner's latest complete collection is also stored independently from later UI
