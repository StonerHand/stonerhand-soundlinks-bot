...
WEBHOOK_BASE_URL=...
STATS_PATH=...
# Redis (shared cache, history/queue, alert dedup — the crate works without it):
UPSTASH_REDIS_REST_URL=...
UPSTASH_REDIS_REST_TOKEN=...
# Branded photo frame (opt-in):
BRAND_PHOTO_FRAME=1
BRAND_LOGO_URL=...
BRAND_LABEL=@stonerhand
```

After changing env, **Redeploy** on Vercel — variables are only picked up by a
new deployment. Check `GET /api/health` returns `"ok": true` (Telegram,
webhook, Redis, queue backlog).
