...
- поддерживает понятный конструктор, восстановление последней карточки и историю;
- запоминает стиль, хэштеги и набор площадок для следующих постов;
...
небольшое число хэштегов, одну основную площадку и кнопку полного релиза.
Остальные действия открываются по контексту. Несколько ссылок не цитируются
повторно и не засоряют итоговый пост. Редактор меняет то же сообщение,
настройки подтверждаются коротким уведомлением, а удаление можно отменить.

Стартовый экран сразу показывает три допустимых входа: ссылку на релиз или
артиста, запрос `артист — название` и несколько ссылок для подборки. Новый
поиск всегда остаётся главным действием; возврат к незавершённой карточке
показывается отдельно и никогда не заменяет основной сценарий.
