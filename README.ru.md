...
- **Поиск** — текстом с выбором из нескольких релизов, ссылкой или прямо из буфера
  (Студия сама предложит вставить скопированную ссылку)
...
- **Оформление** — один шит с тумблерами в стиле лампового усилителя: хэштеги, цитата,
  📸 фото-режим, размер превью + свой текст, свои теги и набор/порядок кнопок платформ
...
- 🔎 **Поиск без ссылки** — напиши `артист - трек` в личку
- 🪄 **Inline** — `@StonerHandBot запрос` в любом чате → выбор из трёх релизов с обложками
- 🎚 **Редактор в чате** — компактные тумблеры и 📤 «В канал» прямо под постом
...
- ⚡ **Живая загрузка** — «⏳ собираю пост…» превращается в карточку
...
- **CI** — GitHub Actions гоняет 259 тестов, линтер, проверку JS и хедлесс-смоук Студии на каждый пуш
...
- **Алерты владельцу** — если health-проверка падает, отложенный пост не ушёл или
  webhook начал крашиться, бот сам пишет тебе в личку (не чаще раза в час на проблему)
...
webapp/index.html    Mini App «Студия» — один файл, vanilla JS
src/music_links_bot/
...
```bash
PYTHONPATH=src python -m pytest tests/   # 259 тестов, без сети
```
