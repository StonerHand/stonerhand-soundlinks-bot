...
| --- | --- | --- |
| 🎛 | **Mini App «Студия»** | Полноценный визуальный редактор внутри Telegram: живой предпросмотр с обложкой, тумблеры, отправка или публикация в один тап — открывается из кнопки меню и из редактора |
...
| 🪄 | **Inline-режим** | Набери `@StonerHandBot <ссылка>` (или название трека) в *любом* чате — выбери из трёх вариантов и вставь готовый пост, не выходя из разговора |
...
| --- | --- |
| Личный чат | Живой плейсхолдер превращается в карточку с панелью редактора: хэштеги, цитата, превью, публикация |
...
| Inline (`@bot ссылка` или название) | Резолвит на лету, предлагает до трёх вариантов с обложками, вставляет полный пост с кнопками |
...
| Кроссплатформенные ссылки | Song.link / Odesli API |
| Поиск по названию | iTunes Search API (публичный, без ключа) |
| Легкие метаданные | Spotify, YouTube и SoundCloud oEmbed, NTS Open Graph |
| Общий кеш и черновики | Upstash Redis / Vercel KV по REST (опционально, мягкая деградация) |
| Языки интерфейса | RU/EN каталог в `i18n.py`, выбор по языку Telegram пользователя |
| Деплой | Vercel webhook (тёплый reuse инстансов) или Railway worker |
| Конфигурация | Environment variables и опциональный `.env` |
| Тестирование | 189 `unittest`-тестов и compile checks, без сети |
...
src/music_links_bot/
├── bot.py            Telegram handlers, routing, клавиатуры, редактор, inline
├── songlink.py       Song.link client, регионы, обложки, Redis-кеш
├── search.py         iTunes Search: текст → кандидаты релизов
├── kvstore.py        Upstash/Vercel KV REST-клиент (опционально)
...
| Preview | Приоритетная платформа управляет preview и порядком кнопок; обложка релиза подстраховывает карточки без превью |
| Гарантия Spotify | Каждая музыкальная карточка начинается с кнопки 🟢 Spotify — при отсутствии прямой ссылки подставляется поисковый deep-link |
| Черновики редактора | Черновики в личке живут в памяти с записью в Redis — тумблеры переживают перезапуски инстансов |
...
В репозитории есть агентные скиллы в `skills/`. Это короткие операционные инструкции для будущей поддержки, чтобы не перечитывать весь код каждый раз заново.
...
`/app` — самодостаточное Telegram Mini App: вставь ссылку или набери название, получи живой предпросмотр карточки (обложка, CTA, хэштеги, кнопки площадок), пощёлкай тумблеры и отправь пост себе — или, как владелец бота, опубликуй сразу в канал с защитой от дублей. Открывается из кнопки меню возле поля ввода (синхронизируется автоматически) и из кнопки 🎛 в редакторе поста. Бэкенд `/api/webapp` проверяет подпись Telegram `initData` — подделать запрос нельзя.
...
| `WEBAPP_URL` | нет | Явный URL Mini App «Студия»; по умолчанию `https://<production-домен>/app` на Vercel |
