...
только данные релиза, теги и действия. Быстрая карточка ограничена четырьмя
действиями: до двух приоритетных площадок, «Все платформы» и контекстное меню
«Ещё». Отправка с сохранением кнопок, добавление в подборку и подходящие
настройки оформления раскрываются только по запросу. Для релиза, найденного по
названию, рядом остаются действия «Другой релиз» и «Изменить запрос». В самой подборке
сначала выбирается номер трека, поэтому интерфейс не разрастается; очистка
требует подтверждения, а случайно удалённый трек можно вернуть.
...
Главное меню собрано в сетку из четырёх действий: **Студия**, **Найти**,
**Подборка** и **Ещё**. Короткий тур показывается только при первом запуске.
