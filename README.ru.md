...
- 🎛 **Студия** — Mini App внутри Telegram: живой предпросмотр, выбор из нескольких найденных релизов, ▶ 30-сек прослушка прямо на обложке, свой текст и хэштеги, набор и порядок кнопок платформ, ↗️ шаринг в любой чат, умная вставка из буфера и «Отменить» после публикации
...
```bash
PYTHONPATH=src python -m pytest tests/   # 231 тест, без сети
```
