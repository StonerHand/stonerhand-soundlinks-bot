...
- Сохраняет точный результат текстового поиска и прямую Apple Music-ссылку с
  ...
  только по точному совпадению артиста и названия в MusicBrainz.
...
- Каждые шесть часов проверяет живые контракты Spotify, MusicBrainz, SoundCloud,
  YouTube, Apple Music и NTS, чтобы автоматически замечать изменения внешних API.
...
подборку одним компактным запросом. Последняя полная подборка владельца хранится
отдельно от следующих действий, поэтому Telegram-обрезание можно восстановить и
после перезапуска; непроверенный список не публикуется как будто полный пост.
