...
- 🎛 **Студия** — Mini App внутри Telegram: живой предпросмотр, выбор из нескольких найденных релизов, ▶ 30-сек прослушка прямо на обложке, свой текст и хэштеги, набор и порядок кнопок платформ
...
```bash
PYTHONPATH=src python -m pytest tests/   # 222 теста, без сети
```
